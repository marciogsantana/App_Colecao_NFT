...
const OPENSEA_LINK = ""
const TOTAL_MINT_COUNT = 50  // numero maximo de NFTS mintadas
...
const App = () => {
   ...
   * Só uma variável de estado que usamos pra armazenar nossa carteira pública. Não esqueça de importar o useState.
     varial de estado para armazenar nossa carteira publica
   */
  const [currentAccount, setCurrentAccount] = useState("")

   // funcão assincrona para esperar retorno da carteira
  const checkIfWalletIsConnected = async () => {
    /*
     * Primeiro tenha certeza que temos acesso a window.ethereum
       verificamos se temos acesso ao objeto window.ethereum
     */
  ...
  const askContractToMintNft = async () => {
    try {
      ...
      if (ethereum) {
        ...
        let nftTxn = await connectedContract.makeAnEpicNFT()  // nftTxn recebe contrato
        console.log("Cunhando...espere por favor.")
        await nftTxn.wait()
        console.log(`Cunhado, veja a transação: https://rinkeby.etherscan.io/tx/${nftTxn.hash}`) // pegamos o hash da transação
      } else {
        console.log("Objeto ethereum não existe!")
      }
    } catch (error) {
