...
contract MyEpicNFT is ERC721URIStorage {
  // verificar funcionanmento dos NFTS
  ...
  string[] firstWords = ["SORTE", "SAUDE", "SABEDORIA", "SUCESSO", "BELEZA", "CONFIANCA"];
  string[] secondWords = ["FORCA", "DETERMINACAO", "CORAGEM", "CHARME", "CARISMA", "FORCA"];
  string[] thirdWords = ["DETERMINACAO", "INTELIGENCIA", "FELICIDADE", "HUMOR", "COMPREENSAO", "SIMPLES"];

  // evento para pegar o endereço e código do token
  event NewEpicNFTMinted(address sender, uint256 tokenId);  


  // Nós precisamos passar o nome do nosso token NFT e o símbolo dele.
  constructor() ERC721 ("SANTANA", "SANT") {
    console.log("Meu contrato de NFT!");
  }
  ...
  // função para usuario chamar  para para pegar uma NFT.
  function makeAnEpicNFT() public {
    ...
    _tokenIds.increment();
    // visualizar NFT mintada
    console.log("Um NFT com ID %s foi cunhado para %s", newItemId, msg.sender);

    emit NewEpicNFTMinted(msg.sender, newItemId);  // emite o evento
  }
